...
pub async fn get_aggregation(ifindex: u32) -> Result<bool, Error> {
...
pub async fn set_aggregation(ifindex: u32, enabled: bool) -> Result<(), Error> {
    let mut attrs = netlink::GenlAttrBuilder::new();
    attrs
        .add(
            Attribute::BatadvAttrMeshIfindex,
    ...
    attrs
        ...
        .map_err(|_| {
            Error::Netlink("Error - could not set Aggregated OGMs attribute".to_string())
        })?;
